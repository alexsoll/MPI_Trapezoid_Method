...
void TParser::SendError(int errNum)
{
   static char *errs[7] = { NULL,
                            NULL,
							const_cast<char *>("Unexpected end of expression"),
							const_cast<char *>("End of expression expected"),
							const_cast<char *>("'(' or '[' expected"),
							const_cast<char *>("')' or ']' expected"),
                            NULL
                         };
...
int test_func(const char* formula, double x1) {
	TParser parser;
	char expr[255];
	static double x[10];
	parser.SetX((double *)&x);
	char *f = const_cast<char *>(formula);
	x[0] = x1;
	parser.Compile(const_cast<char *>("exp(x[0])"));
	parser.Evaluate();
...
int main(void)
{
	...
	cout << "Welcome to Solver!" << endl;
	cout << "TIP: Enter 'x[N]=DDD' to change variables. 0 <= N <= 9." << endl << endl;
	...
	{
		// Get expression
		cout << "Enter expression to parse (or 'quit' to stop):" << endl;
		cin.getline(expr, 255);

		// if 'quit' quit.
		if (!strcmp(expr, "quit")) break;
		...
		if (strchr(expr, '='))
		{
			...
			sscanf_s(expr, "x[%d]=%lf", &_n, &_x);
			if (_n < 0 || _n>9)
			{
				cout << "N out of range. Specify 0 <= N <= 9!" << endl << endl;
				continue;
			}

			x[_n] = _x;

			cout << "x[" << _n << "] set to " << _x << endl << endl;
